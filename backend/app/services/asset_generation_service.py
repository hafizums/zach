...
def generate_mock_image(project: VideoProject, script: Script, scene: Scene, prompt: ImagePrompt) -> GeneratedImageCreate:
    """
    Deterministically generates a mock image using a data URI SVG so frontend can render it without files.
    """
    job_id = f"mock-img-{uuid.uuid4().hex[:8]}"
...
def generate_mock_clip(project: VideoProject, script: Script, scene: Scene, prompt: VideoPrompt, source_image: GeneratedImage) -> GeneratedClipCreate:
    """
    Deterministically generates a mock clip placeholder path.
    """
    job_id = f"mock-vid-{uuid.uuid4().hex[:8]}"
    
    # A placeholder string for the video file path since browsers can't easily play dataURI videos
    file_url = f"/storage/projects/{project.id}/clips/scene_{scene.scene_number}_clip_{job_id}.mp4"
